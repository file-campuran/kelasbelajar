    ...
    <div class="grid grid-cols-12 gap-6">
        <div class=" col-span-4">
            <div class="w-full bg-white p-4 flex items-center  rounded-xl shadow-md">
                ...
                @else
                <img src="{{ asset('Album-Foto-siswa/'.$user_detail->photo) }}"
                    class="bg-gray-400 rounded-full w-16 h-16" style=";max-height: 100px;max-width: 100px;">
                @endif
                <div class="grid md:flex md:flex-col">
                    ...
                    <form action="{{ url('/edit_profile', $user_detail->user_id)}}">
                        <button
                            class=" text-xs py-2  md:py-1 md:text-sm w-full  bg-red-400 rounded-lg capitalize font-semibold  text-white">
                            Edit Profile
                        </button>
                    </form>
                </div>
    ...
    <div class="md:col-span-2">
        <div class="w-full h-full relative flex flex-col md:justify-end">
            <div class="flex items-center ">
                ...
                @else
                <img src="{{ asset('Album-Foto-Guru/'.$user_detail->photo) }}"
                    class="bg-gray-400 rounded-full w-16 h-16" style=";max-height: 100px;max-width: 100px;">
                @endif
...
<div id="kelas" class="md:p-6 w-full lg:p-8 p-4">
    ...
    <h2 class="lg:text-lg text-base capitalize font-semibold text-blue-600">
        Tahun Ajaran {{$setting_semester->tahun_akademik->tahun_akademik}} - semester
        {{$setting_semester->semester->nama_semester}}
    </h2>
    @else
    @endif
    <div class="grid md:gap-8 lg:gap-6 gap-4 md:grid-cols-2 mt-4 md:mt-6 lg:mt-8 lg:grid-cols-3">
        ...
        <div
            ...
            <div class="header uppercase text-center font-bold">
                {{$row->master_kelas->kode_kelas->kode}} {{$row->master_kelas->jurusan->jurusan}}
                {{$row->master_kelas->kelas}} {{$row->tahun_akademik_id}}
            </div>
